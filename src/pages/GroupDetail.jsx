...
function getFileUrl(file) {
  ...
  if (f.startsWith('files/files/')) {
    return BASE_STATIC + '/' + f;
  }
  if (f.startsWith('files/')) {
    return BASE_STATIC + '/files/' + f;
  }
  return BASE_STATIC + '/files/files/' + f;
}

// Video obyektidan URL olish (barcha mumkin bo'lgan maydonlarni tekshirish)
function extractVideoUrl(v) {
  if (!v) return null;
  const raw = v.url || v.file_url || v.path || v.link || v.file_path ||
    v.src || v.video_url || v.stream_url || v.hls_url || v.download_url ||
    v.media_url || v.filepath || v.filename || v.file_name || v.original_name;
  return raw ? getFileUrl(String(raw)) : null;
}
...
function ScheduleTable({ schedules, teacherName, group, students, navigate }) {
  ...
  function handleDateClick(dateStr) {
    ...
    // Faqat bugungi kun uchun AttendancePage ga o'tamiz
    if (dateStr === todayStr && navigate) {
      navigate('/classes/' + (group && group.id) + '/attendance/' + dateStr);
      return;
    }
    // O'tgan kunlar uchun inline (read-only) panel
  ...
  // Faqat bugungi kun uchun tahrirlash mumkin
          ...
          <div key={gIdx} style={{ marginBottom:28 }}>
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:8, marginBottom:8 }}>
              {mg.dates.map(function(item, dIdx) {
                ...
                var isActuallyToday = ds === (function(){
...
function VideoUploadModal({ groupId, groupLessons, setGroupLessons, onClose, onUploaded }) {
  ...
  async function handleUpload() {
    ...
    for (var i = 0; i < files.length; i++) {
      ...
      fd.append('title', item.videoName || item.file.name);
      fd.append('name', item.videoName || item.file.name);
      fd.append('lesson_id', item.lessonId);
      fd.append('lessonId', item.lessonId);
      try {
        var queryParams = '?lessonId=' + item.lessonId + 
                          '&lesson_id=' + item.lessonId + 
                          '&title=' + encodeURIComponent(item.videoName || item.file.name) + 
                          '&name=' + encodeURIComponent(item.videoName || item.file.name);
        var r = await fetch(BASE + '/files/group/' + groupId + '/upload' + queryParams, {
    ...
    if (errors.length > 0) {
      ...
      alert('✓ Videolar muvaffaqiyatli yuklandi!');
      onUploaded();
    }
  }
    ...
    <div style={{ background:'#f1f5f9', paddingBottom:24 }}>
      ...
      {playingVideo && (
        <div style={{ position:'fixed', inset:0, zIndex:2000, display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
          ...
          <div style={{ position:'relative', background:'#000', borderRadius:12, overflow:'hidden', width:'100%', maxWidth:800, zIndex:1 }}>
            ...
            {(function() {
              var videoUrl = extractVideoUrl(playingVideo);
              var fileName = playingVideo.title || playingVideo.name || playingVideo.filename ||
                playingVideo.original_name || playingVideo.file_name || 'Video';
              if (videoUrl) {
                return (
                  <>
                    <video
                      ...
                      onError={function(e){
                        ...
                        var fb = document.getElementById('video-fallback-' + playingVideo.id);
                        if (fb) fb.style.display = 'flex';
                      }}
                    />
                    <div id={'video-fallback-' + playingVideo.id}
                      style={{ display:'none', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'40px', gap:12 }}>
                      <div style={{ fontSize:40 }}>📹</div>
                      <p style={{ color:'#9ca3af', fontSize:13, textAlign:'center' }}>Video yuklanmadi.</p>
                      <a href={videoUrl} target="_blank" rel="noreferrer"
                        style={{ padding:'8px 20px', background:'#7c4dff', color:'white', borderRadius:8, fontSize:13, fontWeight:600, textDecoration:'none' }}>
                        Yuklab olish
                      </a>
                    </div>
                  </>
              ...
              return (
                <div style={{ padding:'40px', textAlign:'center', color:'#9ca3af', fontSize:13 }}>
                  ...
                  <small style={{ color:'#6b7280', fontSize:11, display:'block', marginTop:8 }}>
                    Mavjud maydonlar: {Object.keys(playingVideo).join(', ')}
                  </small>
                </div>
              );
            })()}
      ...
      {activeTab === "Ma'lumotlar" && (
        <div>
          ...
          <ScheduleTable schedules={schedules} teacherName={teacherName} group={group} students={students} navigate={navigate} />
        </div>
      )}
                ...
                <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
                  ...
                  <tbody>
                    {videosLoading ? (
                      ...
                      var videoUrl = extractVideoUrl(v);
                      return (
                        <tr key={v.id||idx} style={{ borderBottom:'1px solid #f5f5f7' }}
                          ...
                          <td style={{ padding:'14px 16px' }}>
                            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                              <div
                                onClick={function(){ setPlayingVideo(v); }}
                                style={{ width:32,height:32,borderRadius:'50%',background:'#7c4dff',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0, cursor:'pointer' }}>
