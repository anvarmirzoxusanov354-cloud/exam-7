...
const AttendancePage = () => {
  ...
  const pageDate = new Date(date + 'T00:00:00');
  const isToday = pageDate.toDateString() === today.toDateString();
  const isPast = pageDate < today;

  // Can edit if:
  // 1. It is today
  // 2. AND either the lesson hasn't been created yet, OR it was created less than 30 minutes ago!
  let canEdit = isToday;
  let timeRemainingStr = '';

  if (isToday && existingLessonId && lessonTime) {
    const timeDiffMs = new Date() - lessonTime;
    const timeDiffMins = timeDiffMs / 1000 / 60;
    if (timeDiffMins > 30) {
      canEdit = false;
    } else {
      const remainingMins = Math.ceil(30 - timeDiffMins);
      timeRemainingStr = `(${remainingMins} daqiqa qoldi)`;
    }
  }
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      let studentList = [];
      if (studentsData) {
        studentList = parseGroupStudents(studentsData);
      }
      if (!studentList.length && groupData) {
        studentList = parseGroupStudents(groupData);
        if (!studentList.length && Array.isArray(groupData.students)) {
          studentList = groupData.students;
        }
      }
      ...
      if (studentList.length > 0) {
        ...
        const init = {};
        studentList.forEach(s => { if (s && s.id != null) init[s.id] = false; });
        setAttendance(init);
      }
      ...
      if (lessonData) {
        ...
        if (Array.isArray(lessonData)) {
          ...
          if (Array.isArray(lessonData.data)) {
            lesson = lessonData.data[0];
          } else {
        ...
        if (lesson && lesson.id) {
          ...
          const createdTime = lesson.created_at || lesson.createdAt || lesson.updated_at || lesson.updatedAt;
          if (createdTime) {
            setLessonTime(new Date(createdTime));
          }

          // Mavjud davomat ma'lumotlarini ham yuklash
          if (lesson.id && studentList.length > 0) {
            fetch(BASE + '/attendance?lessonId=' + lesson.id, { headers })
              .then(r => r.ok ? r.json() : null)
              .then(attData => {
                if (!attData) return;
                const attList = Array.isArray(attData) ? attData
                  : (attData.data || attData.attendance || attData.items || []);
                if (attList.length > 0) {
                  const attMap = {};
                  studentList.forEach(s => { attMap[s.id] = false; });
                  attList.forEach(a => {
                    const sid = a.student_id || (a.student && a.student.id);
                    if (sid != null) attMap[sid] = a.isPresent === true || a.is_present === true;
                  });
    ...
    try {
      ...
      if (!lessonId) {
        try {
          ...
          console.log('[Davomat] lesson tekshiruv status:', checkRes.status);
          if (checkRes.ok) {
            const checkData = await checkRes.json();
            console.log('[Davomat] mavjud lesson:', checkData);
        ...
        } catch(e) { console.warn('[Davomat] lesson tekshiruvda xato:', e); }
      }

      // 2. Lesson yo'q bo'lsa — yangi yaratamiz
      // POST /api/v1/groups/{groupId}/lesson → { group_id, topic, description }
      if (!lessonId) {
        console.log('[Davomat] yangi lesson yaratish:', { group_id: groupId, topic: topic.trim() });
        ...
        console.log('[Davomat] lesson yaratish status:', r.status);
        if (r.ok) {
          const d = await r.json();
          console.log('[Davomat] yaratilgan lesson:', d);
          ...
          console.error('[Davomat] lesson yaratish xatosi:', r.status, errText);
      ...
      console.log('[Davomat] talabalar davomati yuborilmoqda, lessonId:', lessonId);
        ...
        students.map(s =>
          fetch(BASE + '/attendance', {
            ...
            if (!res.ok) {
              ...
              console.error('[Davomat] talaba', s.id, 'xato:', res.status, t);
            } else {
              console.log('[Davomat] talaba', s.id, 'OK, keldi:', attendance[s.id]);
            }
            return res;
          })
        )
      ...
      console.error('[Davomat] umumiy xato:', e);
    ...
    <div style={{ minHeight:'100%', background:'#f1f5f9' }}>
      ...
      {!canEdit && (
        <div style={{ textAlign:'center', padding:'12px', color:'#9ca3af', fontSize:12.5, marginTop:-16 }}>
          {isPast ? "Bu o'tgan kun uchun davomat ko'rish rejimida (tahrirlash mumkin emas)" : "Bu sana uchun davomat qilib bo'lmaydi (kelajak sana)"}
        </div>
      )}
    </div>
  );
