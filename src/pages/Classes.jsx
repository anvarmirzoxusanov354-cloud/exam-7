...
const Classes = () => {
  ...
  useEffect(() => {
    ...
    // Swagger: GET /api/v1/groups/all — faol guruhlar
    // Swagger: GET /api/v1/groups/archive — arxiv guruhlar
    Promise.all([
      fetch(`${BASE_URL}/groups/all`, { headers }).then(r => r.ok ? r.json() : null).catch(() => null),
      fetch(`${BASE_URL}/groups/archive`, { headers }).then(r => r.ok ? r.json() : null).catch(() => null),
      ...
      function mapGroup(g, isActive) {
        return {
          id: g.id,
          status: isActive ? 'FAOL' : 'ARXIV',
      ...
      const activeList = parseList(activeData).map(g => mapGroup(g, g.is_active !== false));
      const archiveList = parseList(archiveData).map(g => mapGroup(g, false));
      // Dublikatlarni oldini olish — arxivdagilarga active:false set qilamiz
      const activeIds = new Set(activeList.map(g => g.id));
      const uniqueArchive = archiveList.filter(g => !activeIds.has(g.id));
      const all = [...activeList, ...uniqueArchive];
      if (all.length > 0) setGroups(all);
    }).catch(() => {});
