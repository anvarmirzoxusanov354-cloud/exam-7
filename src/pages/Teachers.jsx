...
const Teachers = () => {
        ...
        if (res.ok) {
          ...
          const mapped = list.map((u, i) => ({
            ...
            labels: u.groups
              ...
              : [],
            guruhIds: u.groups
              ? u.groups.map(g => typeof g === 'object' ? (g.id || g._id) : g)
              : [],
            jinsi: u.gender || '',
            selected: false,
            archived: false,
  ...
  const archiveConfirmed = async (id) => {
  ...
  const handleSave = async () => {
    ...
    // Tug'ilgan sanani ISO formatga o'tkazish (YYYY-MM-DD)
    let isoDate = null;
    if (form.born) {
    ...
    if (editId !== null) {
      ...
      try {
        ...
        if (form.jinsi) {
          const genderVal = form.jinsi === 'Erkak' ? 'male' : form.jinsi === 'Ayol' ? 'female' : form.jinsi;
          formData.append('gender', genderVal);
        }
        if (form.guruhIds && form.guruhIds.length > 0) {
          form.guruhIds.forEach(id => formData.append('groups[]', id));
        }
        ...
        if (res.ok) {
          setTeachers(prev => prev.map(t => t.id === editId ? {
            ...t, name: form.name.trim(), phone: cleanPhone, born: form.born, email: form.email,
            guruh: form.guruhlar[0] || '', labels: form.guruhlar, guruhIds: form.guruhIds, jinsi: form.jinsi,
          } : t));
        } else {
          try {
      ...
      try {
        ...
        formData.append('phone', cleanPhone);
        if (form.email) formData.append('email', form.email);
        formData.append('password', form.parol || 'Teacher123!');
        if (isoDate) formData.append('birth_date', isoDate);
        if (form.jinsi) {
          const genderVal = form.jinsi === 'Erkak' ? 'male' : form.jinsi === 'Ayol' ? 'female' : form.jinsi;
          formData.append('gender', genderVal);
            ...
            <div className="bg-white rounded-[16px] shadow-[0_20px_60px_rgba(0,0,0,0.15)] max-w-[400px] w-full overflow-hidden">
              <div className="px-6 pt-6 pb-2">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <h2 className="m-0 text-[17px] font-bold text-[#1a1a2e]">O'qituvchini o'chirish</h2>
                </div>
              ...
              <div className="px-6 py-5 flex gap-3 justify-end">
                ...
                <button
                  onClick={() => archiveConfirmed(deleteConfirm.id)}
                ...
                <tr key={t.id} className={`border-b border-[#f9f9fb] transition-colors duration-150 ${rowIdx % 2 === 0 ? 'bg-white' : 'bg-[#fafafa]'} hover:bg-[#f5f0ff]`}>
                  ...
                  <td className="p-2.5">
                    <div className="flex gap-1 items-center">
                      <button onClick={() => archiveOne(t.id, t.name)} title="Arxivga o'tkazish" className="bg-none border-none cursor-pointer text-[#ef5350] flex p-[3px] hover:text-[#e53935]"><DeleteOutlineOutlined style={{ fontSize: '16px' }} /></button>
