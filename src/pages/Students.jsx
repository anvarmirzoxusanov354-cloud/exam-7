...
import {
  ...
  LocationOnOutlined, VisibilityOutlined, RefreshOutlined,
...
const Students = () => {
  ...
  const requestDeleteOne = (id, name) => {
    setDeleteConfirm({ id, name });
  };

  const archiveConfirmedStudent = async (id) => {
    const token = localStorage.getItem('accessToken');
    try {
    ...
    setStudents(prev => {
      ...
      if (found) setArchivedStudents(arch => [...arch, { ...found, archived: true }]);
  ...
  const restoreStudent = (id) => {
    setArchivedStudents(prev => {
      const found = prev.find(s => s.id === id);
      if (found) setStudents(list => [{ ...found, archived: false }, ...list]);
      return prev.filter(s => s.id !== id);
    });
  };
    ...
    <div className="p-2 sm:p-4 lg:p-6 bg-[#f1f5f9] h-full flex flex-col overflow-hidden">
      ...
      {deleteConfirm && (
        <>
          <div
            ...
            className="fixed inset-0 z-[1400] bg-black/40 backdrop-blur-[2px]"
          />
          <div className="fixed inset-0 z-[1410] flex items-center justify-center p-4">
            <div className="bg-white rounded-[16px] shadow-[0_20px_60px_rgba(0,0,0,0.15)] max-w-[400px] w-full overflow-hidden">
              <div className="px-6 pt-6 pb-2">
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-10 h-10 rounded-full bg-[#fee2e2] flex items-center justify-center shrink-0">
                    <DeleteOutlineOutlined style={{ fontSize: 20, color: '#ef5350' }} />
                  </div>
                  <h2 className="m-0 text-[17px] font-bold text-[#1a1a2e]">Talabani o'chirish</h2>
                </div>
                <p className="m-0 text-[13.5px] text-[#6b7280] leading-[1.6] pl-[52px]">
                  <span className="font-semibold text-[#1a1a2e]">{deleteConfirm.name}</span> ni rostdan ham o'chirishni hohlaysizmi?
                  <br />
                  <span className="text-[12.5px]">O'chirilgan ma'lumot arxivga o'tkaziladi.</span>
                </p>
              </div>
              <div className="px-6 py-5 flex gap-3 justify-end">
                <button
                  onClick={() => setDeleteConfirm(null)}
                  className="px-5 py-2 rounded-[10px] border border-[#e5e7eb] bg-white text-[#374151] font-semibold text-[13px] cursor-pointer hover:bg-[#f9fafb] transition-colors"
                >
                  Bekor qilish
                </button>
                <button
                  onClick={() => archiveConfirmedStudent(deleteConfirm.id)}
                  className="px-5 py-2 rounded-[10px] border-none bg-[#ef5350] text-white font-semibold text-[13px] cursor-pointer hover:bg-[#e53935] transition-colors"
                >
                  Ha, o'chirish
                </button>
              </div>
      ...
      {groupsModalOpen && (
        <div
          ...
          className="fixed inset-0 z-[1250] bg-black/40 backdrop-blur-[2px] transition-opacity duration-300"
        />
      )}

      {/* Groups Selection Modal - Centered */}
      {groupsModalOpen && <div className="fixed inset-0 z-[1260] flex items-center justify-center">
                ...
                <tr key={s.id} className={`border-b border-[#f9f9fb] transition-colors ${idx % 2 === 0 ? 'bg-white' : 'bg-[#fafafa]'} hover:bg-[#f5f0ff]`}>
                  ...
                  <td className="p-[10px_10px]">
                    <div className="flex gap-1 items-center">
                      ...
                      <button onClick={() => requestDeleteOne(s.id, s.name)} title="O'chirish" className="bg-transparent border-none cursor-pointer text-[#ef5350] flex p-[3px] hover:text-[#e53935]">
      ...
      {archivedStudents.length > 0 && (
        <div className="bg-white rounded-[16px] shadow-[0_1px_8px_rgba(0,0,0,0.06)] mt-4 overflow-x-auto shrink-0">
          <div className="flex items-center gap-2 p-[14px_18px] border-b border-[#f1f1f5]">
            ...
            <span className="font-semibold text-[14px] text-[#1a1a2e]">Arxivdagi talabalar</span>
            <span className="bg-[#ede9ff] text-[#7c4dff] text-[12px] font-bold px-2 py-0.5 rounded-[6px]">{archivedStudents.length}</span>
          </div>
          <table className="w-full border-collapse text-[13px] min-w-[700px]">
            <thead>
              <tr className="border-b border-[#f1f1f5]">
                ...
                <th className="p-[12px_10px] text-left font-semibold text-[#374151] text-[12.5px]">Email</th>
                <th className="p-[12px_10px] w-28"></th>
              </tr>
            </thead>
            <tbody>
              {archivedStudents.map((s, i) => (
                <tr key={s.id || i} className={`border-b border-[#f9f9fb] ${i % 2 === 0 ? 'bg-white' : 'bg-[#fafafa]'} hover:bg-[#f5f0ff]`}>
                  <td className="p-[10px_14px]">
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 rounded-full bg-[#ede9ff] flex items-center justify-center font-bold text-[13px] text-[#7c4dff] shrink-0">
                        {s.name?.charAt(0)?.toUpperCase() || 'T'}
                      </div>
                      <span className="font-semibold text-[#1a1a2e] text-[13px]">{s.name}</span>
                    </div>
                  </td>
                  <td className="p-2.5 text-[#374151]">{s.phone}</td>
                  <td className="p-2.5 text-[#6b7280]">{(s.guruhlar || []).join(', ') || '—'}</td>
                  <td className="p-2.5 text-[#6b7280]">{s.email || '—'}</td>
                  <td className="p-2.5 text-right">
                    ...
                    >
                      <RefreshOutlined style={{ fontSize: 14 }} /> Qaytarish
                    </button>
                  </td>
                </tr>
              ))}
