...
const Sidebar = ({ onClose, isCollapsed }) => {
  ...
  return (
    <div className={`${isCollapsed ? 'w-20 p-2' : 'w-72 lg:w-64 p-4'} ${darkMode ? 'bg-[#1f1f23]' : 'bg-white'} flex flex-col h-screen sticky top-0 rounded-r-3xl transition-all duration-300`}>
      ...
      <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'} mb-10 px-2`}>
        <div className="flex items-center gap-2">
          ...
          {!isCollapsed && <span className={`text-xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>Najotedu</span>}
        </div>
        {!isCollapsed && (
          <button onClick={onClose} className={`lg:hidden p-2 ${darkMode ? 'text-zinc-400 hover:text-zinc-200' : 'text-gray-400 hover:text-gray-600'}`}>
            <CloseOutlined />
          </button>
        )}
      </div>
            ...
            <Link
              ...
              className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-3 px-4'} py-3 rounded-xl transition-all no-underline
                ${active
                  ...
                  : darkMode ? 'text-zinc-400 hover:bg-[#27272a] hover:text-zinc-200' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-700'
                }`}
            >
              <Icon fontSize="small" className={active ? 'text-white' : darkMode ? 'text-zinc-500' : 'text-gray-400'} />
              {!isCollapsed && (
                <span className={`text-sm font-semibold ${active ? 'text-white' : darkMode ? 'text-zinc-300' : 'text-gray-600'}`}>
                  {label}
                </span>
              )}
      ...
      {!isCollapsed && (
        <div className="mt-auto">
          <div className={`${darkMode ? 'bg-[#18181b]' : 'bg-gray-50'} p-4 rounded-2xl`}>
            <div className="flex gap-3 mb-4">
              <div className={`p-2 ${darkMode ? 'bg-[#1f1f23]' : 'bg-white'} rounded shadow-sm`}>
                <DescriptionOutlined className="text-orange-400" fontSize="small" />
              </div>
              <div>
                <p className={`text-xs font-bold ${darkMode ? 'text-white' : ''}`}>{t('subscription')}</p>
